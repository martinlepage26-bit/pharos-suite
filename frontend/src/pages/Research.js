  ...
  return (
    <div className="min-h-screen bg-[#f8f9fc] py-12 px-6 md:px-12" data-testid="research-page">
      <div className="max-w-5xl mx-auto">
        <h1 className="font-serif text-4xl md:text-5xl font-semibold text-[#1a2744] mb-4">{t.research.title}</h1>
        <p className="text-gray-600 mb-8 max-w-2xl">{t.research.description}</p>

        <div className="mb-12">
          <h2 className="font-serif text-2xl font-semibold text-[#1a2744] mb-4">{t.research.howItWorks}</h2>
          <div className="flex items-center justify-center gap-4 text-sm mb-6">
            {[t.research.signal, t.research.pressure, t.research.control, t.research.artifact, t.research.evidence].map((label, i, arr) => (
              <span key={i}>
                <span className="text-[#6366f1] font-medium">{label}</span>
        ...
        <div className="mb-8">
          <div className="flex flex-col md:flex-row md:items-center md:justify-between mb-6">
            <h2 className="font-serif text-2xl font-semibold text-[#1a2744]">{t.research.operationalContexts}</h2>
            <p className="text-gray-500 text-sm">{t.research.filterByContext}</p>
          </div>
          <div className="flex flex-wrap gap-2 mb-8">
            <button onClick={() => setSelectedContext('all')}
              className={`px-4 py-2 rounded-full text-sm font-medium transition-all ${selectedContext === 'all' ? 'bg-[#6366f1] text-white' : 'bg-white border border-gray-200 text-gray-600 hover:border-[#6366f1] hover:text-[#6366f1]'}`}>
              {t.research.all}
            </button>
            {contexts.map((context) => (
              <button key={context.id} onClick={() => setSelectedContext(context.id)} data-testid={`context-${context.id}`}
                className={`px-4 py-2 rounded-full text-sm font-medium transition-all ${selectedContext === context.id ? 'bg-[#6366f1] text-white' : 'bg-white border border-gray-200 text-gray-600 hover:border-[#6366f1] hover:text-[#6366f1]'}`}>
                {context.title}
              </button>
            ))}
          </div>
        ...
        <div className="space-y-4">
          <h3 className="font-serif text-lg font-semibold text-[#1a2744] mb-4">{t.research.briefings} ({filteredPapers.length})</h3>
              ...
              <div className="flex items-start justify-between gap-4">
                <div className="flex-1">
                  <div className="flex items-center gap-2 mb-2">
                    ...
                    <span className="text-xs font-medium text-[#6366f1] uppercase tracking-wide">{paper.type}</span>
                  ...
                  <h4 className="font-serif text-lg font-semibold text-[#1a2744] mb-2 group-hover:text-[#6366f1] transition-colors">{paper.title}</h4>
        ...
        <>
          ...
          <div className="fixed top-0 right-0 h-screen w-full md:w-[600px] bg-white border-l border-gray-200 shadow-2xl z-[4500] flex flex-col overflow-hidden">
            <div className="flex items-start justify-between gap-4 p-6 border-b border-gray-100 bg-white">
              <div>
                <div className="flex items-center gap-2 mb-2">
                  <span className="text-xs font-medium text-[#6366f1] uppercase tracking-wide">{selectedPaper.type}</span>
                  <span className="text-gray-300">·</span>
                ...
                <h2 className="font-serif text-2xl font-semibold text-[#1a2744]">{selectedPaper.title}</h2>
              </div>
