    ...
    <div className="min-h-screen bg-[#F6F7FB] py-12 px-6" data-testid="tool-page">
      <div className="max-w-[980px] mx-auto mb-6">
        ...
        <div className="flex gap-4 mt-4 text-xs tracking-wider uppercase">
          <span className={step >= 1 ? 'text-[#1A1050] font-semibold' : 'text-gray-400'}>{t.tool.steps.sector}</span>
          <span className="text-gray-300">·</span>
          <span className={step >= 2 ? 'text-[#1A1050] font-semibold' : 'text-gray-400'}>{t.tool.steps.readiness}</span>
          <span className="text-gray-300">·</span>
          <span className={drawerOpen ? 'text-[#1A1050] font-semibold' : 'text-gray-400'}>{t.tool.steps.results}</span>
        </div>
      </div>
          ...
          <div className="relative bg-white/95 border border-gray-200/60 rounded-3xl shadow-[0_4px_20px_rgba(11,15,26,0.06)] p-5 overflow-visible">
            ...
            {step === 1 && (
              <div data-testid="step-1-sector">
                <div className="flex items-center gap-3 mb-2">
                  <span className="w-7 h-7 rounded-full bg-[#1A1050] text-white flex items-center justify-center font-bold text-sm">1</span>
                  ...
                  {sectorKeys.map((key, i) => (
                    <button key={key} onClick={() => handleSectorSelect(sectorIds[i])} data-testid={`sector-${sectorIds[i]}`}
                      className={`w-full text-left p-3 rounded-2xl border transition-all duration-150 shadow-[0_2px_8px_rgba(11,15,26,0.04)] hover:translate-y-[-1px] hover:shadow-[0_3px_10px_rgba(11,15,26,0.06)] ${
                        selectedSector === sectorIds[i] ? 'border-[#1A1050]/55 shadow-[0_3px_12px_rgba(75,42,191,0.12)] bg-[#1A1050]/5' : 'border-gray-200/60 bg-white/90 hover:border-[#1A1050]/25'
                      }`}
                    >
                      <h3 className="font-bold text-[#0B0F1A]/90 mb-1 text-sm">{t.tool.sectors[key].title}</h3>
                      <p className="text-[#0B0F1A]/70 text-sm leading-snug">{t.tool.sectors[key].description}</p>
                    </button>
                  ))}
                </div>
              </div>
            ...
            {step === 2 && (
              <div data-testid="step-2-readiness">
                <div className="flex items-center gap-3 mb-2">
                  <span className="w-7 h-7 rounded-full bg-[#1A1050] text-white flex items-center justify-center font-bold text-sm">2</span>
                  ...
                  {currentQ.options.map((option) => (
                    <button key={option.value} onClick={() => handleAnswer(currentQ.id, option.value, option.score)}
                      className={`rounded-full border py-2.5 px-3 font-bold text-sm transition-all duration-150 hover:translate-y-[-1px] hover:shadow-[0_3px_10px_rgba(11,15,26,0.05)] ${
                        answers[currentQ.id]?.value === option.value ? 'border-[#1A1050]/55 shadow-[0_3px_10px_rgba(75,42,191,0.10)] bg-[#1A1050] text-white' : 'border-gray-200/60 bg-white/90 hover:border-[#1A1050]/25'
                      }`}
                    >{option.label}</button>
                  ))}
                </div>
                <div className="flex justify-center gap-1.5 mt-4">
                  {READINESS_QUESTIONS.map((_, i) => (
                    <div key={i} className={`w-2 h-2 rounded-full transition-colors ${i === currentQuestion ? 'bg-[#1A1050]' : answers[READINESS_QUESTIONS[i].id] ? 'bg-[#1A1050]/40' : 'bg-gray-300'}`} />
                  ))}
                </div>
              </div>
            )}
            ...
            <div className="flex items-center gap-2.5 mt-4 pt-4 border-t border-gray-100">
              {(step > 1 || (step === 2 && currentQuestion > 0)) && (
                <button onClick={handleBack} className="text-gray-600 hover:text-[#1A1050] font-medium text-sm" data-testid="back-btn">{t.tool.buttons.back}</button>
              )}
              ...
              <button onClick={handleNext} disabled={!canProceed} data-testid="next-btn"
                className={`ml-auto px-5 py-2 rounded-full font-semibold text-sm transition-all ${canProceed ? 'bg-[#1A1050] text-white hover:bg-[#3A1FA0]' : 'bg-gray-200 text-gray-400 cursor-not-allowed'}`}
              >
            ...
            {step === 1 && !selectedSector && (
              <p className="text-[#1A1050]/80 font-semibold text-sm mt-2.5">{t.tool.buttons.selectSector}</p>
            )}
          </div>
        </div>
      </div>
        ...
        <>
          ...
          <div className="fixed top-0 right-0 h-screen w-[min(440px,92vw)] bg-white/95 border-l border-gray-200/60 shadow-[0_4px_24px_rgba(11,15,26,0.10)] z-[4500] flex flex-col" data-testid="results-drawer">
            <div className="flex items-start justify-between gap-3 p-4 border-b border-gray-100">
              <div>
                <p className="text-xs tracking-widest uppercase text-[#1A1050]">{t.tool.steps.results}</p>
                  ...
                  <>
                    <div className="grid grid-cols-[120px_1fr] gap-4 items-center p-3 border border-gray-200/60 rounded-2xl bg-[#f6f7fb]/70">
                      <div className="w-[110px] h-[110px] rounded-full grid place-items-center shadow-[0_3px_10px_rgba(75,42,191,0.10)]"
                        style={{ background: `conic-gradient(#1A1050 ${percentage * 3.6}deg, rgba(11,15,26,0.06) ${percentage * 3.6}deg 360deg)` }}>
                      ...
                      <div>
                        ...
                        <p className="font-serif font-bold text-xl text-[#1A1050] mb-1">{risk}</p>
                    ...
                    <div className="mt-3 p-3 border border-gray-200/60 rounded-2xl bg-white/90">
                      ...
                      <div className="flex items-center justify-between mt-3 pt-3 border-t border-gray-100 font-bold">
                        ...
                        <span className="text-[#1A1050]">{score}/{total}</span>
                      </div>
                    </div>
                    <div className="mt-3 p-3 border border-gray-200/60 rounded-2xl bg-white/90">
                    ...
                    <div className="flex flex-wrap gap-2 mt-4">
                      ...
                      <button onClick={handleReset} className="text-gray-500 hover:text-[#1A1050] text-sm ml-auto">{t.tool.results.retake}</button>
                    </div>
                  </>
                );
              })()}
