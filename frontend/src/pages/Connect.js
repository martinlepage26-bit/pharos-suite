  ...
  return (
    <div className="min-h-screen bg-[#F6F7FB] py-12 px-6 md:px-12" data-testid="connect-page">
      <div className="max-w-5xl mx-auto">
        <h1 className="font-serif text-4xl md:text-5xl font-semibold text-[#0B0F1A] mb-4">{t.connect.title}</h1>
        ...
        <div className="flex gap-2 mb-8">
          <button onClick={() => { setActiveTab('message'); setBookingSubmitted(false); }} className={`px-4 py-2 rounded-full text-sm font-medium transition-all ${activeTab === 'message' ? 'bg-[#180E66] text-white' : 'bg-white border border-gray-200 text-gray-600 hover:border-[#180E66]'}`} data-testid="tab-message">
            <Mail className="w-4 h-4 inline mr-2" />{t.connect.tabs.message}
          </button>
          <button onClick={() => setActiveTab('booking')} className={`px-4 py-2 rounded-full text-sm font-medium transition-all ${activeTab === 'booking' ? 'bg-[#180E66] text-white' : 'bg-white border border-gray-200 text-gray-600 hover:border-[#180E66]'}`} data-testid="tab-booking">
            ...
            {activeTab === 'message' ? (
              <div className="card">
                <div className="flex items-center gap-3 mb-6">
                  <div className="w-10 h-10 rounded-xl bg-[#180E66]/10 flex items-center justify-center"><Mail className="w-5 h-5 text-[#180E66]" /></div>
                  <div>
                ...
                <form onSubmit={handleSubmit} data-testid="contact-form">
                  <div className="grid grid-cols-2 gap-4 mb-4">
                    <div>
                      ...
                      <input type="text" name="name" value={formData.name} onChange={handleChange} required className="w-full px-4 py-3 border border-gray-200 rounded-lg focus:border-[#180E66] focus:outline-none" data-testid="input-name" />
                    </div>
                    <div>
                      <label className="block text-sm font-medium text-gray-700 mb-1">{t.connect.form.email} *</label>
                      <input type="email" name="email" value={formData.email} onChange={handleChange} required className="w-full px-4 py-3 border border-gray-200 rounded-lg focus:border-[#180E66] focus:outline-none" data-testid="input-email" />
                    </div>
                  </div>
                  <div className="mb-4">
                    <label className="block text-sm font-medium text-gray-700 mb-1">{t.connect.form.organization}</label>
                    <input type="text" name="organization" value={formData.organization} onChange={handleChange} className="w-full px-4 py-3 border border-gray-200 rounded-lg focus:border-[#180E66] focus:outline-none" />
                  </div>
                  <div className="mb-4">
                    <label className="block text-sm font-medium text-gray-700 mb-1">{t.connect.form.lookingFor}</label>
                    <select name="lookingFor" value={formData.lookingFor} onChange={handleChange} className="w-full px-4 py-3 border border-gray-200 rounded-lg focus:border-[#180E66] focus:outline-none" data-testid="input-looking-for">
                  ...
                  <div className="mb-6">
                    ...
                    <textarea name="context" value={formData.context} onChange={handleChange} placeholder={t.connect.form.contextPlaceholder} rows={4} className="w-full px-4 py-3 border border-gray-200 rounded-lg focus:border-[#180E66] focus:outline-none resize-none" data-testid="input-context" />
                  </div>
              ...
              <div className="card" data-testid="booking-calendar-card">
                <div className="flex items-center gap-3 mb-6">
                  <div className="w-10 h-10 rounded-xl bg-[#180E66]/10 flex items-center justify-center"><CalendarIcon className="w-5 h-5 text-[#180E66]" /></div>
                  <div>
                ...
                {bookingSubmitted ? (
                  <div className="text-center py-8" data-testid="booking-confirmation">
                    ...
                    <button onClick={() => setBookingSubmitted(false)} className="mt-4 text-[#180E66] font-medium hover:underline text-sm">
                                ...
                                <button
                                  ...
                                  className={`py-2 px-3 rounded-lg text-sm font-medium transition-all border ${
                                    isBooked
                                      ...
                                      : selectedTime === time
                                        ? 'bg-[#180E66] text-white border-[#180E66]'
                                        : 'bg-white text-gray-700 border-gray-200 hover:border-[#180E66] hover:text-[#180E66]'
                                  }`}
                                >
                                  {time}
                    ...
                    <div className="border-t border-gray-100 pt-6">
                      ...
                      <div className="grid grid-cols-2 gap-4 mb-4">
                        <div>
                          ...
                          <input type="text" name="name" value={bookingForm.name} onChange={handleBookingFormChange} required className="w-full px-4 py-3 border border-gray-200 rounded-lg focus:border-[#180E66] focus:outline-none" data-testid="booking-name" />
                        </div>
                        <div>
                          <label className="block text-sm font-medium text-gray-700 mb-1">{t.bookingCalendar.email} *</label>
                          <input type="email" name="email" value={bookingForm.email} onChange={handleBookingFormChange} required className="w-full px-4 py-3 border border-gray-200 rounded-lg focus:border-[#180E66] focus:outline-none" data-testid="booking-email" />
                        </div>
                      </div>
                      <div className="mb-4">
                        <label className="block text-sm font-medium text-gray-700 mb-1">{t.bookingCalendar.organization}</label>
                        <input type="text" name="organization" value={bookingForm.organization} onChange={handleBookingFormChange} className="w-full px-4 py-3 border border-gray-200 rounded-lg focus:border-[#180E66] focus:outline-none" data-testid="booking-org" />
                      </div>
                      <div className="mb-4">
                        <label className="block text-sm font-medium text-gray-700 mb-1">{t.bookingCalendar.topic}</label>
                        <select name="topic" value={bookingForm.topic} onChange={handleBookingFormChange} className="w-full px-4 py-3 border border-gray-200 rounded-lg focus:border-[#180E66] focus:outline-none" data-testid="booking-topic">
                      ...
                      <div className="mb-6">
                        ...
                        <textarea name="currentState" value={bookingForm.currentState} onChange={handleBookingFormChange} placeholder={t.bookingCalendar.currentStatePlaceholder} rows={3} className="w-full px-4 py-3 border border-gray-200 rounded-lg focus:border-[#180E66] focus:outline-none resize-none" data-testid="booking-state" />
                      </div>
                      <button
                        type="submit"
            ...
            {resources.map((resource, index) => (
              <div key={index} className="card card-hover" data-testid={`resource-${index}`}>
                <div className="flex items-start gap-3">
                  <div className="w-8 h-8 rounded-lg bg-[#180E66]/10 flex items-center justify-center flex-shrink-0"><resource.icon className="w-4 h-4 text-[#180E66]" /></div>
                  <div>
                    ...
                    <button onClick={() => handleResourceRequest(resource.title)} className="text-[#180E66] font-medium text-xs hover:underline">{t.connect.resources.request}</button>
                  </div>
                </div>
              </div>
            ))}
            <div className="p-4 bg-[#180E66]/5 rounded-xl border border-[#180E66]/20 mt-6">
              <p className="text-sm text-gray-600">
                <span className="font-semibold text-[#0B0F1A]">{t.connect.resources.directContact}</span><br />
                <a href="mailto:martin@martinlepage.com" className="text-[#180E66] hover:underline">martin@martinlepage.com</a>
              </p>
            </div>
            <Link to="/tool" className="block p-4 bg-[#180E66] text-white rounded-xl hover:bg-[#180E66] transition-colors">
