    ...
    return (
      <div className="min-h-screen bg-[#F6F7FB] flex items-center justify-center px-6" data-testid="admin-login">
        <form onSubmit={handleLogin} className="card max-w-sm w-full text-center">
          ...
          <input type="password" value={passphrase} onChange={(e) => setPassphrase(e.target.value)} placeholder="Passphrase" className="w-full px-4 py-3 border border-gray-200 rounded-lg focus:border-[#180E66] focus:outline-none mb-4" data-testid="admin-passphrase" />
      ...
      <div className="max-w-5xl mx-auto">
        ...
        <div className="flex gap-2 mb-8">
          <button onClick={() => setActiveTab('publications')} data-testid="admin-tab-publications"
            className={`px-4 py-2 rounded-full text-sm font-medium transition-all flex items-center gap-2 ${activeTab === 'publications' ? 'bg-[#180E66] text-white' : 'bg-white border border-gray-200 text-gray-600 hover:border-[#180E66]'}`}>
            <BookOpen className="w-4 h-4" /> {t.admin.publications}
          </button>
          <button onClick={() => setActiveTab('bookings')} data-testid="admin-tab-bookings"
            className={`px-4 py-2 rounded-full text-sm font-medium transition-all flex items-center gap-2 ${activeTab === 'bookings' ? 'bg-[#180E66] text-white' : 'bg-white border border-gray-200 text-gray-600 hover:border-[#180E66]'}`}>
        ...
        {activeTab === 'publications' && (
          <div>
            ...
            {showForm && (
              <div className="card mb-6 border-l-4 border-[#180E66]" data-testid="publication-form">
                ...
                <form onSubmit={handlePubSubmit} className="space-y-4">
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      ...
                      <input type="text" value={pubForm.title} onChange={e => setPubForm(p => ({ ...p, title: e.target.value }))} required className="w-full px-3 py-2 border border-gray-200 rounded-lg focus:border-[#180E66] focus:outline-none text-sm" data-testid="pub-title" />
                    </div>
                    <div>
                      <label className="block text-sm font-medium text-gray-700 mb-1">{t.admin.type}</label>
                      <input type="text" value={pubForm.type} onChange={e => setPubForm(p => ({ ...p, type: e.target.value }))} placeholder="Protocol, Briefing, Working Paper..." className="w-full px-3 py-2 border border-gray-200 rounded-lg focus:border-[#180E66] focus:outline-none text-sm" />
                    </div>
                  </div>
                  <div className="grid grid-cols-3 gap-4">
                    <div>
                      <label className="block text-sm font-medium text-gray-700 mb-1">{t.admin.venue}</label>
                      <input type="text" value={pubForm.venue} onChange={e => setPubForm(p => ({ ...p, venue: e.target.value }))} className="w-full px-3 py-2 border border-gray-200 rounded-lg focus:border-[#180E66] focus:outline-none text-sm" />
                    </div>
                    <div>
                      <label className="block text-sm font-medium text-gray-700 mb-1">{t.admin.year}</label>
                      <input type="text" value={pubForm.year} onChange={e => setPubForm(p => ({ ...p, year: e.target.value }))} className="w-full px-3 py-2 border border-gray-200 rounded-lg focus:border-[#180E66] focus:outline-none text-sm" />
                    </div>
                    <div>
                      <label className="block text-sm font-medium text-gray-700 mb-1">{t.admin.status}</label>
                      <select value={pubForm.status} onChange={e => setPubForm(p => ({ ...p, status: e.target.value }))} className="w-full px-3 py-2 border border-gray-200 rounded-lg focus:border-[#180E66] focus:outline-none text-sm">
                  ...
                  <div>
                    ...
                    <textarea value={pubForm.description} onChange={e => setPubForm(p => ({ ...p, description: e.target.value }))} rows={2} className="w-full px-3 py-2 border border-gray-200 rounded-lg focus:border-[#180E66] focus:outline-none text-sm resize-none" />
                  </div>
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      <label className="block text-sm font-medium text-gray-700 mb-1">{t.admin.link}</label>
                      <input type="text" value={pubForm.link} onChange={e => setPubForm(p => ({ ...p, link: e.target.value }))} placeholder="/sealed-card or https://..." className="w-full px-3 py-2 border border-gray-200 rounded-lg focus:border-[#180E66] focus:outline-none text-sm" />
                    </div>
                  ...
                  <div key={pub.id} className="card flex items-start justify-between gap-4" data-testid={`admin-pub-${pub.id}`}>
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2 mb-1">
                        {pub.type && <span className="text-xs font-medium text-[#180E66] uppercase tracking-wide">{pub.type}</span>}
                    ...
                    <div className="flex gap-1 flex-shrink-0">
                      <button onClick={() => handleEditPub(pub)} className="p-2 hover:bg-[#180E66]/10 rounded-lg transition-colors" data-testid={`edit-pub-${pub.id}`}>
                        <Pencil className="w-4 h-4 text-gray-500 hover:text-[#180E66]" />
                      </button>
