  ...
  return (
    <div className="min-h-screen bg-[#F6F7FB] py-12 px-6 md:px-12" data-testid="portfolio-page">
      <div className="max-w-4xl mx-auto">
        <div className="mb-12">
          <Link to="/about" className="text-[#4B2ABF] text-sm font-medium hover:underline mb-4 inline-block">&larr; Back to About</Link>
        ...
        <section className="mb-12">
          <h2 className="font-serif text-2xl font-semibold text-[#0B0F1A] mb-6 flex items-center gap-3">
            <BookOpen className="w-6 h-6 text-[#4B2ABF]" /> Publications & Frameworks
          </h2>
          {loading ? (
                    ...
                    <div className="flex items-start justify-between gap-4">
                      <div>
                        <div className="flex items-center gap-2 mb-2">
                          <span className="text-xs font-medium text-[#4B2ABF] uppercase tracking-wide">{pub.type}</span>
                        ...
                        <h3 className="font-serif text-lg font-semibold text-[#0B0F1A] mb-2 group-hover:text-[#4B2ABF] transition-colors">{pub.title}</h3>
                        <p className="text-gray-600 text-sm">{pub.description}</p>
                      </div>
                      <ExternalLink className="w-5 h-5 text-gray-400 group-hover:text-[#4B2ABF] flex-shrink-0" />
                    </div>
                  </Link>
                ) : (
        ...
        {workingPapers.length > 0 && (
          <section className="mb-12">
            <h2 className="font-serif text-2xl font-semibold text-[#0B0F1A] mb-6 flex items-center gap-3">
              <FileText className="w-6 h-6 text-[#4B2ABF]" /> Working Papers
            </h2>
            <div className="space-y-4">
              {workingPapers.map((paper, i) => (
                <div key={paper.id} className="card bg-[#F6F7FB]" data-testid={`working-paper-${i}`}>
                  <div className="flex items-center gap-2 mb-2">
                    <span className="text-xs font-medium text-[#4B2ABF] uppercase tracking-wide">In development</span>
                  </div>
        ...
        <section className="mb-12">
          <h2 className="font-serif text-2xl font-semibold text-[#0B0F1A] mb-6 flex items-center gap-3">
            <Presentation className="w-6 h-6 text-[#4B2ABF]" /> Engagement Areas
          </h2>
          <div className="space-y-4">
            {engagements.map((eng, i) => (
              <div key={i} className="card" data-testid={`engagement-${i}`}>
                <div className="flex items-start gap-4">
                  <div className="w-12 h-12 rounded-xl bg-[#4B2ABF]/10 flex items-center justify-center flex-shrink-0"><eng.icon className="w-6 h-6 text-[#4B2ABF]" /></div>
                  <div className="flex-1">
                    <div className="flex items-center gap-2 mb-1">
                      <span className="text-xs font-medium text-[#4B2ABF] uppercase tracking-wide">{eng.type}</span>
                      <span className="text-gray-300">·</span>
          ...
          <div className="grid md:grid-cols-2 gap-4">
            {expertise.map((area, i) => (
              <div key={i} className="card">
                <h3 className="font-medium text-[#4B2ABF] text-sm uppercase tracking-wide mb-3">{area.category}</h3>
                <div className="flex flex-wrap gap-2">
