  ...
  return (
    <div className="min-h-screen bg-[#F6F7FB] py-12 px-6 md:px-12" data-testid="portfolio-page">
      <div className="max-w-4xl mx-auto">
        <div className="mb-12">
          <Link to="/about" className="text-[#0D0A2E] text-sm font-medium hover:underline mb-4 inline-block">&larr; Back to About</Link>
                    ...
                    <div className="flex items-start justify-between gap-4">
                      <div>
                        <div className="flex items-center gap-2 mb-2">
                          <span className="text-xs font-medium text-[#0D0A2E] uppercase tracking-wide">{pub.type}</span>
            ...
            <div className="space-y-4">
              {workingPapers.map((paper, i) => (
                <div key={paper.id} className="card bg-[#F6F7FB]" data-testid={`working-paper-${i}`}>
                  ...
                  <p className="text-gray-600 text-sm whitespace-pre-line">{formatAdminTextForDisplay(paper.description)}</p>
                </div>
              ))}
            </div>
          </section>
        ...
        <section className="mb-12">
          <h2 className="font-serif text-2xl font-semibold text-[#0B0F1A] mb-6 flex items-center gap-3">
            <Presentation className="w-6 h-6 text-[#0D0A2E]" /> Engagement Areas
          </h2>
          <div className="space-y-4">
            {engagements.map((eng, i) => (
              <div key={i} className="card" data-testid={`engagement-${i}`}>
                <div className="flex items-start gap-4">
                  <div className="w-12 h-12 rounded-xl bg-[#0D0A2E]/10 flex items-center justify-center flex-shrink-0"><eng.icon className="w-6 h-6 text-[#0D0A2E]" /></div>
                  <div className="flex-1">
                    <div className="flex items-center gap-2 mb-1">
                      <span className="text-xs font-medium text-[#0D0A2E] uppercase tracking-wide">{eng.type}</span>
                      <span className="text-gray-300">·</span>
          ...
          <div className="grid md:grid-cols-2 gap-4">
            {expertise.map((area, i) => (
              <div key={i} className="card">
                <h3 className="font-medium text-[#0D0A2E] text-sm uppercase tracking-wide mb-3">{area.category}</h3>
                <div className="flex flex-wrap gap-2">
        ...
        <div className="card bg-[linear-gradient(135deg,#0D0A2E_0%,#0D0A2E_40%,#1A1555_70%,#2D2380_100%)] text-white shadow-[0_8px_32px_rgba(42,32,107,0.4)] relative overflow-hidden">
